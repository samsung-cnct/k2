...
If it becomes necessary to make a breaking change, a new schema with a new version must be created. Previous schema versions should be supported for at least two k2 releases or 3 months (whichever is longer). During this time, attempts to use configuration files generated by older k2 releases will fail and a depreciation notice will be printed. In order to continue using these configuration files the --force flag must be passed to k2cli.
